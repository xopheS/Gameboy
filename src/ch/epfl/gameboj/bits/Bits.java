...
public final class Bits {
     ...
     *             si l'index est invalide (s'il n'est pas compris entre 0 (inclus)
     *             et 32 (exclus))
     */
    ...
    public static boolean test(int bits, int index) {
        if ((bits >> Objects.checkIndex(index, Integer.SIZE)) % 2 == 0) {
            return false;
        }
        return true;
    }
    ...
    public static boolean test(int bits, Bit bit) {
        if ((bits >> Objects.checkIndex(bit.index(), Integer.SIZE)) % 2 == 0) {
            return false;
        }
        return true;
    }
     ...
     * @param newValue
     *            nouvelle valeur
     * @return un entier dont tous les bits sont égaux à ceux de l'entier bits sauf
     *         le bit d'index 'index' qui sera égal à 'newValue'
     * @throws IndexOutOfBoundsException
     *             si l'index est invalide (s'il n'est pas compris entre 0 (inclus)
     *             et 32 (exclus))
     */
    public static int set(int bits, int index, boolean newValue) {
        if (newValue) {
            if (!test(bits, Objects.checkIndex(index, Integer.SIZE))) {
     ...
     * @param bits
     *            l'entier à traiter
     * @return retourne une valeur dont les 'size' bits de poids faible sont égaux à
     *         ceux de 'bits', les autres valant 0
     * @throws IndexOutOfBoundsException
     ...
     *            l'entier à traiter
     * @return retourne une valeur dont les 'size' bits de poids faible sont égaux à
     *         ceux de bits allant de l'index 'start' (inclus) à l'index 'start +
     *         size' (exclus)
     * @throws IndexOutOfBoundsException
    ...
    /**
     * Applique une rotation vers la droite ou vers la gauche des size bits de poids
     * faible de bits si la distance est positive, la rotation se fait vers la
     * gauche, sinon elle se fait vers la droite.
     *
     * @param size
     ...
     * @return Retourne une valeur dont les size bits de poids faible sont ceux de
     *         bits mais auxquels une rotation de la distance donnée a été appliquée
     * @throws IllegalArgumentException
     *             si size n'est pas compris entre 0 (exclus) et 32 (inclus), ou si
    ...
    /**
     * Copie le bit d'index 7 dans les bits d'index 8 à 31 de la valeur retournée
     * (étend le signe de b).
     *
     * @param b
     *            l'entier
    ...
    /**
     * Renverse la valeur des 8 bits de l'entier b (échange les valeurs des bits 7
     * et 0, 6 et 1, 5 et 2, 4 et 3).
     *
     * @param b
     *            l'entier
     * @return Retourne une valeur égale à b mais dont les bits d'index 0 et 7 ont
     *         été échangés, de même que ceux d'index 1 et 6, 2 et 5, et 3 et 4
     * @throws IllegalArgumentException
     *             si la valeur donnée n'est pas une valeur de 8 bits 
     */
    public static int reverse8(int b) {
        int[] reverse = new int[] { 0x00, 0x80, 0x40, 0xC0, 0x20, 0xA0, 0x60, 0xE0, 0x10, 0x90, 0x50, 0xD0, 0x30, 0xB0,
     ...
     * @param lowB
     *            entier conportant les bits de poids fort
     * @return une valeur 16 bits dont les 8 bits de poids forts sont les 8 bits de
     *         poids faible de highB, et dont les 8 bits de poids faible sont ceux
     *         de lowB
     * @throws IllegalArgumentException
     *             si la valeur donnée n'est pas une valeur de 8 bits 
     */
     ...
     * @param intList
     *            liste d'entier
     * @return Retourne un entier résultant de l'application d'un 'ou' entre tous
