...
/**
 * Cette classe, immuable, simule un vecteur de bits plus long que 64,
 * qui est la taille maximale en bits d'un type intégral Java (le type long).
 *
...
public final class BitVector {
    // Un vecteur de bits à zéro partout d'une longueur égale à la largeur de l'écran de la GameBoy
    ...
    /**
     * Cette méthode construit un tableau d'entiers de la taille donnée,
     * tous les bits étant à la valeur initiale spécifiée.
     * 
     * @param size
    ...
    /**
     * Cette classe utilitaire modélise une division euclidienne,
     * en emballant le quotient et le reste dans un même objet.
     * 
     * @author Christophe Saad (282557)
     * @author David Cian (287967)
     *
     ...
     * spécifie la nature de l'extraction: extension par zéro ou enroulement
    ...
    private BitVector extract(int start, int size, boolean methodZeroExtended) {
        ...
        // Le quotient représente l'index de début mesuré en blocs de 32 bits (la taille en bits d'un entier)
        ...
        if (methodZeroExtended) {
            ...
            if (Math.floorMod(start, Integer.SIZE) == 0) {
                for (int i = 0; i < extractedInts.length; i++) {
                    /* Si l'index du bloc du vecteur extrait décalé de l'index du bloc de départ n'appartient pas
                     * à l'intervalle [0, longueur du vecteur[, le bloc extrait est nul, sinon c'est celui à l'index décalé
                     */
            ...
            if (Math.floorMod(start, Integer.SIZE) == 0) {
                for (int i = 0; i < extractedInts.length; i++) {
                    // Le bloc extrait est égal à celui de même index modulo la longueur du vecteur d'origine
    ...
    /**
     * Cette méthode extrait un bloc (un entier) de ce vecteur, au moyen d'une extension par zéro.
     * 
     * @param start
    ...
    private int intExtZeroExtended(int start) {
        // Le quotient de la division représente l'index de départ mesuré en blocs (entiers)
        FloorEuclideanDiv f = new FloorEuclideanDiv(start, Integer.SIZE);
        if (f.div >= bitVector.length || f.div < -1) {
            // Si l'index de départ n'appartient pas à [0, longueur du vecteur[, le bloc retourné est nul
            return 0;
        } else {
    ...
    /**
     * Cette méthode extrait un bloc (un entier) de ce vecteur, au moyen d'un enroulement.
     * 
     * @param start
     * l'index de départ, en bits
     * @return l'entier extrait
    ...
    /**
     * Constructeur utilitaire, qui assigne un tableau d'entiers au tableau sous-jacent.
     * 
     * @param bitVector
    ...
    /**
     * Constructeur qui crée un vecteur de bits de taille et valeur initiale données.
     * 
     * @param size
    ...
    /**
     * Cette méthode permet d'obtenir un masque des MSB de la longueur du vecteur, à partir d'un index donné.
     * 
     * @param index
    ...
    /**
     * Cette méthode permet d'obtenir un masque des LSB de la longueur du vecteur, à partir d'un index donné.
     * 
     * @param index
     * l'index de démarcation
     * @return le masque
    ...
    /**
     * Cette méthode permet d'obtenir seulement les MSB d'un vecteur à partir d'un index donné.
     * 
     * @param index
     * l'index de démarcation
    ...
    /**
     * Cette méthode permet d'obtenir seulement les LSB d'un vecteur à partir d'un index donné.
     * 
     * @param index
     * l'index de démarcation
     * @return le vecteur "coupé"
    ...
    /**
     * Cette méthode permet de donner une certaine valeur aux bits du vecteur sélectionnés par le masque.
     * 
     * @param mask
    ...
    /**
     * Cette méthode permet d'obtenir un vecteur extrait de celui-ci au moyen d'une extension par zéro.
     * 
     * @param start
    ...
    /**
     * Cette méthode permet d'obtenir un vecteur extrait de celui-ci au moyen d'une extension par enroulement.
     * 
     * @param start
     * l'index de départ de l'extraction
     * @param size
    ...
    /**
     * Cette méthode permet d'obtenir un vecteur décalé d'une distance donnée, dans les deux sens.
     * 
     * @param distance
    ...
    /**
     * Cette classe permet de construire un vecteur de bits progressivement (comme celui-ci est immuable).
     * 
     * @author Christophe Saad (282557)
     * @author David Cian (287967)
     *
    ...
    public static final class Builder {
        ...
        public Builder setByte(int index, int b) {
            /* Si l'instance du constructeur a déjà construit un vecteur, il ne peut plus en construire 
             * (mais il est possible de garder la même variable et changer la référence en instanciant un nouveau constructeur)
             */
            if (vector == null) {
