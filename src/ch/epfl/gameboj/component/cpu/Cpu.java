...
public final class Cpu implements Component, Clocked {
    ...
    private static Opcode[] buildOpcodeTable(Opcode.Kind opKind) {
        ...
        for(Opcode o : Opcode.values()) {
            if(o.kind == opKind) opcodeTable[o.encoding] = o;
        }
        return opcodeTable;
    }
    ...
    private void dispatch(Opcode opcode) {          
        ...
        // Subtract
        case SUB_A_R8: {
            int sub = Alu.sub(registers8.get(Reg.A), registers8.get(extractReg(opcode, 0)), getInitialCarry(opcode)); 
        ...
        // Misc. ALU
        case DAA: {
            int adjust = Alu.bcdAdjust(registers8.get(Reg.A), registers8.testBit(Reg.F, Flag.N), registers8.testBit(Reg.F, Flag.H), registers8.testBit(Reg.F, Flag.C));
    ...
    private void setReg16(Reg16 r, int newV) {
        if(r == Reg16.AF) newV &= 0b11111111_11111111_11111111_11110000;
    ...
    private Reg extractReg(Opcode opcode, int startBit) {
        ...
        case 4: return Reg.H;
        case 5: return Reg.L;
        case 6: throw new NullPointerException();
        case 7: return Reg.A;
        }
    ...
    private Reg16 extractReg16(Opcode opcode) {
        ...
        case 2: return Reg16.HL;
        case 3: return Reg16.AF;
        }
    ...
    private void combineAluFlags(int vf, FlagSrc z, FlagSrc n, FlagSrc h, FlagSrc c) { 
        ...
        for (int i = tableFlag.length; i < Flag.values().length; i++) {
            ...
            case V0: {registers8.setBit(Reg.F, Flag.values()[i], false);} break;
            case V1: {registers8.setBit(Reg.F, Flag.values()[i], true);} break;
            case ALU: {registers8.setBit(Reg.F, Flag.values()[i], Bits.test(Alu.unpackFlags(vf), Flag.values()[i]));} break;
            case CPU: {registers8.setBit(Reg.F, Flag.values()[i], Bits.test(registers8.get(Reg.F), Flag.values()[i]));} break;
            }             
        } 
    ...
    private boolean testCondition(Opcode opcode) {
        switch(Bits.extract(opcode.encoding, 3, 2)){
            case 0: return !registers8.testBit(Reg.F, Flag.Z);
            case 1: return registers8.testBit(Reg.F, Flag.Z);
            case 2: return !registers8.testBit(Reg.F, Flag.C);
            case 3: return registers8.testBit(Reg.F, Flag.C);
            default:
            	return false;
        }
    }
    ...
    public int[] _testGetPcSpAFBCDEHL() {
        return new int[] {PC, SP, registers8.get(Reg.A), registers8.get(Reg.F), registers8.get(Reg.B), registers8.get(Reg.C), registers8.get(Reg.D), registers8.get(Reg.E), registers8.get(Reg.H), registers8.get(Reg.L)};
    }
}
