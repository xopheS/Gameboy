...
public final class LcdController implements Component, Clocked {
	...
	private LcdImageLine computeLine(int lineIndex) {
		...
		if (isBackgroundActive()) {
			LcdImageLine nextBGLine = computeBGorWinLine(lineIndex, true).extractWrapped(lcdRegs.get(LCDReg.SCX),
					LCD_WIDTH);
		...
		if (lineIndex >= lcdRegs.get(LCDReg.WY) && isWindowActive(adjustedWX)) {
			nextLine = nextLine.join(computeBGorWinLine(winY, false).extractZeroExtended(-adjustedWX, LCD_WIDTH),
					adjustedWX);
			winY++;
		}
	...
	private LcdImageLine computeSpriteLine(Integer[] spriteInfo, int lineIndex, int spriteHeight) {
		...
		for (int i = 0; i < spriteInfo.length; ++i) {
			...
			int[] tileLineBytes = videoRamController.tileLineBytes(spriteTileIndex,
					spriteTileLineIndex(lineIndex, spriteY, vFlip, spriteHeight));
			...
			spriteLines[i] = spriteLineBuilder.build().shift(-spriteX)
					.mapColors(lcdRegs.get(spritePalette ? LCDReg.OBP1 : LCDReg.OBP0));
		}
	...
	private int getHeight() {
		// Taille des sprites: 8 x 8 ou 8 x 16 pixels
