...
/**
 * Cette classe modélise un contrôleur DMA (Direct Memory Access), composant à part entière de la GameBoy,
 * qui permet d'effectuer une copie rapide (un octet/cycle) de la RAM vers la VRAM/OAM du contrôleur LCD. 
 * 
...
public final class DmaController {
    ...
    void start(int addressMSB) {
        ...
        if (!isActive) {
            ...
            // D'après la documentation de la GameBoy, l'adresse ne peut être donnée que par tranches de 256 (un octet non signé)
    ...
    /**
     * Cette méthode copie un octet à la fois de la RAM vers la VRAM/OAM du contrôleur LCD.
     */
    void copy() {
        if (!isActive)
    ...
    /**
     * Cette méthode incrémente l'index de la copie, en terminant la copie si elle a tout copié.
     */
    ...
    /**
     * Cette méthode met fin à la copie en remettant les attributs du contrôleur à leurs valeurs initiales.
     */
    private void end() {
        isActive = false;
        currentIndex = 0;
